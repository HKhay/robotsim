...
def detect_colored_object():
    """
    Detects only:
    Green/Bright green = plant
    Brown/Red-brown = trash

    For now we ignore compressed objects because dark pixels/shadows
    are being falsely detected as compressed trash.
    """
    ...
    for y in range(y_start, y_end, 3):
        for x in range(x_start, x_end, 3):
            ...
            # Plant cube: green is clearly dominant
            if (
                g > 90 and
                g > r * 1.3 and
                g > b * 1.3
            ...
            # Trash cube: brown/reddish object
            # Avoid detecting grey floor/walls by requiring red dominance
            elif (
                r > 75 and
                g > 20 and
                g < 95 and
    ...
    if len(plant_pixels) > len(trash_pixels):
        object_type = "plant"
        best_pixels = plant_pixels
    else:
        object_type = "trash"
        best_pixels = trash_pixels
